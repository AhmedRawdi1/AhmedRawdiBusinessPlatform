...
BEGIN
    ...
    BEGIN TRY
        IF ISNULL(@GroupID, 0) = 0
            THROW 50001, 'Group ID is required.', 1;

        IF NOT EXISTS (SELECT 1 FROM dbo.SystemGroups WITH (NOLOCK) WHERE ID = @GroupID)
        ...
        -- Parse JSON permissions array into temporary table
        SELECT
            FormID,
        ...
        -- Upsert group form permissions into SystemFormsPermissions table
        MERGE INTO dbo.SystemFormsPermissions WITH (UPDLOCK, HOLDLOCK) AS Target
        USING #ParsedPermissions AS Source
        ON (Target.GroupID = @GroupID 
            AND Target.FormID = Source.FormID 
            AND Target.UserID IS NULL 
    ...
    BEGIN CATCH
        ...
        IF OBJECT_ID('tempdb..#ParsedPermissions') IS NOT NULL
            DROP TABLE #ParsedPermissions;
